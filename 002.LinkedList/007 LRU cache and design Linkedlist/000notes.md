...
# Question: Design Linked List (LeetCode 707) [Medium]
...
### Visual Dry Run & Operation Flow (Singly Linked List)

![Design Singly Linked List Operations & Dry Run](./svg_01_design_singly_linked_list.svg)

---

### Visual Architecture: Doubly Linked List with Sentinel/Dummy Nodes

![Design Doubly Linked List with Dummy Nodes](./svg_02_design_doubly_linked_list.svg)

---

### Handwritten Notes Analysis & Key Takeaways

1. **The Core Invariant — `llSize` tracking**:
   - Tracking `llSize` explicitly prevents infinite loops or out-of-bounds pointer dereferencing.
   - Boundaries:
     - `get(index)`: Valid only if `index >= 0 && index < llSize`. If invalid, return `-1`.
     - `addAtIndex(index, val)`: Valid if `index >= 0 && index <= llSize`. If `index > llSize`, immediately return without inserting.
     - `deleteAtIndex(index)`: Valid only if `index >= 0 && index < llSize`. If `index >= llSize`, immediately return.

2. **Crucial Edge Cases in Deletion**:
   - **Case `llSize == 1`**: Deleting index 0 must set both `head = null` and `tail = null`, and decrement `llSize` to 0.
   - **Case `index == 0` (Delete Head)**: Point `head = head.next`, clean up pointers, and decrement `llSize`.
   - **Case `index == llSize - 1` (Delete Tail)**: In a Singly Linked List, you cannot step backward from `tail`. You must traverse from `head` to `llSize - 2` to update `tail = curr` and `tail.next = null`.
   - **Intermediate Deletion**: Walk to node at `index - 1`, link `node.next = node.next.next`, decrement `llSize`.

3. **Step-by-Step Trace of Example (from Notes)**:
   - Command sequence: `["MyLinkedList", "addAtHead(1)", "addAtTail(3)", "addAtIndex(1, 2)", "get(1)", "deleteAtIndex(0)", "get(0)"]`
   - **Step 1 (`addAtHead(1)`)**: `head = tail = [1]`, `llSize = 1`. List: `1 -> null`.
   - **Step 2 (`addAtTail(3)`)**: `tail.next = [3]`, `tail = [3]`, `llSize = 2`. List: `1 -> 3 -> null`.
   - **Step 3 (`addAtIndex(1, 2)`)**: Node `[2]` inserted between `[1]` and `[3]`, `llSize = 3`. List: `1 -> 2 -> 3 -> null`.
   - **Step 4 (`get(1)`)**: Traverse 1 step from `head`, returns `2` ✅.
   - **Step 5 (`deleteAtIndex(0)`)**: Head `[1]` removed, `head` becomes `[2]`, `llSize = 2`. List: `2 -> 3 -> null`.
   - **Step 6 (`get(0)`)**: Node at index 0 is `[2]`, returns `2` ✅.

4. **Code Quality & Best Practices**:
   - Using `this.val = val;` in constructor prevents variable shadowing and eliminates ambiguity.

---

### Implementation 1: Singly Linked List (Direct Pointers)
...
### Implementation 2: Doubly Linked List with Sentinel / Dummy Nodes (Optimal Pattern)

Using dummy `head` and dummy `tail` completely eliminates all boundary edge cases (such as deleting when `size == 1` or inserting into an empty list).
...
public:
    ...
    int get(int index) {
        ...
        // Optimization: search from head or tail depending on index
        Node* curr;
        if (index < size / 2) {
            curr = head->next;
    ...
    void deleteAtIndex(int index) {
        ...
        if (index < size / 2) {
            ...
            succ = tail;
            for (int i = 0; i < size - 1 - index; i++) succ = succ->prev;
            Node* toDelete = succ->prev;
            pred = toDelete->prev;
            pred->next = succ;
            succ->prev = pred;
            delete toDelete;
...
class MyLinkedList {
    ...
    public int get(int index) {
        ...
        Node curr;
        // Optimization: search from head or tail based on closest distance
        if (index < size / 2) {
            curr = head.next;
            for (int i = 0; i < index; i++) curr = curr.next;
        } else {
...
| Operation | Singly Linked List | Doubly Linked List (with Sentinel) |
|---|---|---|
| `get(index)` | $O(\text{index}) \le O(N)$ | $O(\min(\text{index}, N - \text{index})) \le O(N/2)$ |
| `addAtHead(val)` | $O(1)$ | $O(1)$ |
| `addAtTail(val)` | $O(1)$ (with tail pointer) | $O(1)$ |
| `addAtIndex(index, val)` | $O(\text{index}) \le O(N)$ | $O(\min(\text{index}, N - \text{index})) \le O(N/2)$ |
| `deleteAtIndex(index)` | $O(\text{index}) \le O(N)$ | $O(\min(\text{index}, N - \text{index})) \le O(N/2)$ |
| **Space Complexity** | $O(N)$ total space, $O(1)$ auxiliary | $O(N)$ total space, $O(1)$ auxiliary |
